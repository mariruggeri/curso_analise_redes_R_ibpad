...
df <- read_csv("football-events/events.csv")
#  dataframe de metadados das partidas
df_inf <- read_csv("football-events/ginf.csv")
...
lst_grafo_por_time$`Real Madrid`
# plotar grafos
plotar_grafo <- function(grafo, titulo){
  plot(grafo, edge.arrow.size = 0.1, main = titulo, edge.label = E(grafo)$n)
}
par(mfrow = c(1,2))
...
extrair_centralidade_jogador <- function(grafo, jogador){
  ...
  res <- c(dg, btn, cln)
  names(res) <- c("degree", "betweenness", "closeness")
  return(res)
}
